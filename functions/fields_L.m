function[field]=fields_L(panels,solution,tamany,N)
...
for i=1:N
    for j=1:N
        ...
        if inpolygon(field.x(i,j),field.y(i,j),panels.vertex(:,1),panels.vertex(:,2))
            ...
            field.u(i,j)=(solution.V_inf*cosd(-solution.alpha)-sum((solution.lambda./(2*pi)).*((D.*F)./2+C.*G)))+solution.gamma/(2*pi)*sum(C.*F./2-D.*G);
            field.v(i,j)=-solution.V_inf*sind(-solution.alpha)-sum((solution.lambda./(2*pi)).*(-(C.*F)./2+D.*G))+solution.gamma/(2*pi)*sum(D.*F./2+C.*G);
        end
    end
end
